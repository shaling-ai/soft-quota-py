...
async def test_no_rule_allowed(service):
    """No rule => allowed."""
    result = await service.check_quota("user", "u1", "sms_sent", "per_user")
...
async def test_record_usage_increases_count(service, rule_repo):
    ...
    result2 = await service.check_quota("user", "u1", "sms_sent", "per_user")
...
async def test_different_subjects_independent(service, rule_repo):
    ...
    await service.record_usage("user", "u1", "sms_sent", count=1)
    r1 = await service.check_quota("user", "u1", "sms_sent", "per_user")
    r2 = await service.check_quota("user", "u2", "sms_sent", "per_user")
